...
$.extend(AjaxComboBox.prototype, /** @lends AjaxComboBox.prototype */ {
	...
	_setElem: function(combo_input) {
		...
		elem.combo_input = $(combo_input)
			...
			// .wrap('<div class="oya">'); // This "div" is "container".

		// elem.container = $(elem.combo_input).parent().addClass(this.css_class.container);
		elem.container = $('.shop-search-sr-result');
		...
		elem.navi_info   = $('<div>').addClass('info').attr({"aria-live":"polite"});
		elem.navi_p      = $('<p>');
		elem.results     = $('<ul>' ).addClass(this.css_class.results).attr({"id":"shop_name_list"}).attr({"role":"listbox"});
	...
	_setButtonPosition: function() {
		...
		// $(this.elem.img).css({top: '-50% !important'});
    $(this.elem.img).css({'cssText': 'top: -50% !important;'}).css({left: left,transform:'translate(0, -50%)'});
	},
	...
	_ehComboInput: function() {
		...
		$(self.elem.combo_input)
			...
			.click(function() {
				...
				$(self.elem.results).children('li').removeClass(self.css_class.select).attr({"aria-selected":"false"});
			});
	},
	...
	_ehResults: function() {
		...
		$(self.elem.results)
			...
			.mouseover(function() {
				...
				$(self.elem.results).children('li').removeClass(self.css_class.select).attr({"aria-selected":"false"});
				$(this).addClass(self.css_class.select).attr({"aria-selected":"true"});
				$("#shop_name_1").attr({"aria-activedescendant":$(this).attr("id")});
    ...
    console.log(q_word,which_page_num)
		// データ取得
		if (typeof self.option.source == 'object') self._searchForJson(self, q_word, which_page_num);
		else                                       self._searchForApi(self, q_word, which_page_num);
	},
  ...
  _searchForApi:function(self, q_word, which_page_num) {
      console.log('q_word:',q_word);
      const pageId = Number(which_page_num);
      const host = location.hostname;
      const pathName = location.pathname;
      const pathList = pathName.split('/');
      let isGrandTop = false;
      ...
      if(pathList[1] === 'brands') isGrandTop = true;

      let apiUrl = '/brands';
      if(isGrandTop){
        apiUrl = '/api/v1/brands';
      }else{
        // apiUrl = `/api/v1/${pathList[1]}/brands`;
        apiUrl = `/api/v1/${pathList[1]}/brands`;
      }
      console.log(apiUrl)
      $.ajax({
        ...
        if(isGrandTop){
          for (var i = 0; i < json.length; i++) {
            filterData.push(json[i].base);
          }
        }else{
          filterData = json;
        }
        for (var i = 0; i < q_word.length; i++) {
          filterData = _.filter(filterData, function(filterData) {
            const text_ja_Big = filterData.name;
            const text_ja_Small = filterData.name_m;
            const text_en = filterData.name_en;
            let isResult = false;
            // console.log('text_ja_Big:',text_ja_Big)
            if(text_ja_Big.indexOf(q_word[i]) !== -1 && text_ja_Big.length >= 1) isResult = true;
            // if(text_ja_Small.includes(q_word) && text_ja_Small.length >= 1) isResult = true;
            // if(text_en.indexOf(q_word[i]) !== -1 && text_en.length >= 1) isResult = true;
            if(text_en.toLowerCase().includes(q_word[i].toLowerCase()) && text_en.length >= 1) isResult = true;
            if(text_en.toUpperCase().includes(q_word[i].toUpperCase()) && text_en.length >= 1) isResult = true;
            return isResult;
          });
        }
        filterData = _.sortBy(filterData, ["name_en"])
        const chunkList = _.chunk(filterData, 10)
        // console.log('filterData:',filterData);
        // console.log('pageId:',pageId);
        // console.log('抽出番号:',chunkList[pageId-1])
        const currentChunkList = chunkList[pageId-1];
        const resultList = [];

        if(chunkList.length <= 0) {
          $('#shop_name_list').empty();
          $('.ac_result_area .ac_navi .info').text('1 - 0 件 (全 0 件)');
          return;
        };

        for (var i = 0; i < currentChunkList.length; i++) {
          resultList.push(currentChunkList[i]['name']);
        }
        const resultJson = {
          "cnt_whole": filterData.length,
          "cnt_page": 10,
        ...
        if(resultList.length>=1) self._prepareResults(self, resultJson, q_word, which_page_num);
        console.log('------↑API↑------');
			},
    ...
    console.log('ajax3:(source)',self.option.source)
    console.log('ajax3(q_word)',q_word)
    console.log('ajax3(search_field)',self.option.search_field)
    console.log('ajax3(which_page_num)',which_page_num)
		self.prop.xhr = $.ajax({
			dataType: 'json',
			url: self.option.source,
			data: {
	...
	_prepareResults: function(self, json, q_word, which_page_num) {
		...
		if (self.prop.is_paging === false) {
			...
			$(obj).addClass(self.css_class.select).attr({"aria-selected":"true"});
    ...
    $("#shop_name_1").attr({"aria-expanded":"true"});
	},
	...
	_calcWidthResults: function(self) {
		...
		$(self.elem.container).css('width', '100%');
    ...
    $("#shop_name_1").attr({"aria-expanded":"false"}).removeAttr("aria-activedescendant");
	},
	...
	_nextLine: function(self) {
		...
		if (!obj) {
			...
			var idx = $(self.elem.results).children('li').index(obj);
			$(obj).removeClass(self.css_class.select).attr({"aria-selected":"false"});
		}
		idx++;
		if (idx < $(self.elem.results).children('li').length) {
			var next = $(self.elem.results).children('li').eq(idx);
			self._setSubInfo(self, next);
			$(next).addClass(self.css_class.select).attr({"aria-selected":"true"});
      $("#shop_name_1").attr({"aria-activedescendant":$(next).attr("id")});
			self._setCssFocusedResults(self);
		} else {
			self._setCssFocusedInput(self);
		}
	...
	_prevLine: function(self) {
		...
		if (idx > -1) {
			...
			$(prev).addClass(self.css_class.select).attr({"aria-selected":"true"});
      $("#shop_name_1").attr({"aria-activedescendant":$(prev).attr("id")});
			self._setCssFocusedResults(self);
		} else {
			self._setCssFocusedInput(self);
		}
