...
$.extend(AjaxComboBox.prototype, /** @lends AjaxComboBox.prototype */ {
   ...
   _searchForApi:function(self, q_word, which_page_num) {
      ...
      // console.log('host:',pathList[2])
      if(pathName.indexOf('/en/brands/') > -1 ) isGrandTop = true;
      ...
      if(isGrandTop){
        ...
        apiUrl = `/api/v1/${pathList[2]}/brands`;
      }
      console.log(apiUrl)
      $.ajax({
			dataType: 'json',
        ...
        let filterData = [];
        console.log('isGrandTop:',isGrandTop)
        if(isGrandTop){
        ...
        for (var i = 0; i < q_word.length; i++) {
          filterData = _.filter(filterData, function(filterData) {
            ...
            const text_en_b = filterData.name_en.toUpperCase();
            const text_en_s = filterData.name_en.toLowerCase();
            let isResult = false;
        ...
        if(chunkList.length <= 0) {
          $('.ac_results').empty();
          $('.ac_navi p').empty();
          $('.ac_result_area .ac_navi .info').text('1 - 0 of 0');
          return;
        };
        // console.log('chunkList:',currentChunkList);
        // if(chunkList.length <= 0) return;
        for (var i = 0; i < currentChunkList.length; i++) {
          resultList.push(currentChunkList[i]['name_en']);
  ...
  // }
    ...
    let $btnSearch =$('.shop-search__btn input');
    ...
    $btnSearch.on('click',(e)=>{
      ...
      const pathName = location.pathname;
      if(pathName.indexOf('/en/brands/') > -1 ) isGrandTop = true;
      const $inputText = $('input[name="shop_name_1"]').val();
      const $jsResult = $('.js-result');
      console.log('isGrandTop:',isGrandTop);
      $('.js-shop-container').addClass('is-active');
      if(isGrandTop){
        const $gridList = $jsResult.find('.js-grid-item');
        for (var i = 0; i < $gridList.length; i++) {
          const $glid = $gridList.eq(i);
          const alt = $glid.find('.ciGrid__item__cover img').attr('alt');
          if(alt===$inputText){
            console.log('alt:',alt);
            $glid.show();
          }else{
        ...
        const $framelist = $('tr.brands-framelist__name');
        const $framelistSpan = $framelist.find('span');
        const $frameAlpha = $('.brands-framelist__title--alpha');
        $frameAlpha.hide();
        for (var i = 0; i < $framelist.length; i++) {
          const $frame = $framelist.eq(i);
          const text = $frame.find('.js-name').text();
          // console.log('text:',text);
          if(text===$inputText){
            $frame.show();
            const alphaText = text.substr(0,1).toUpperCase();
            console.log(alphaText);
            $('.brands-framelist__title--alpha.search-'+alphaText).show();
          }else{
            $frame.hide();
          }
        }
      }else{
        const $jsResultBody = $('.js-result-body');
        $('.js-result-head').hide();
        for (var i = 0; i < $jsResultBody.length; i++) {
          const $resultBody =  $jsResultBody.eq(i);
          
          const text =  $resultBody.find('.result__body__shop-name-en').text();
          if(text===$inputText){
            $resultBody.show().addClass('.js-result-current');
            const alphaText = text.substr(0,1).toLowerCase();
            console.log(alphaText);
            $('.result__heading.search-'+alphaText).show();
          }else{
            $resultBody.hide();
          }
        }
      ...
      if(isGrandTop){
        ...
        let sendpath = path;
        if(path.includes('detail_')){
          sendpath = '../brands/';
        }

        if($inputText) {
          sendpath = sendpath+'?'+ 'brandname='+$inputText;
