...
Condition.prototype={init:function($app){var s=this;this.$app=$app;this.$condition=this.$app.find('.js-bus-search-condition');},show:function(condText){var html='';html+='出发日期 : '+(condText.date||'Unspecified')+'<br>';if(!condText.dir){html+='希望乘车时间:Unspecified<br>'}else{if(!condText.time_begin&&!condText.time_end){html+='希望乘车时间 : '+condText.dir+' Unspecified<br>';}else{html+='希望乘车时间 : '+condText.dir+' '+(condText.time_begin||'')+' ~ '+(condText.time_end||'')+'<br>';}}
if(!condText.place[0]){html+='抵达时间 : Unspecified<br>';}else{html+='抵达时间 : '+condText.place.join('、')+'<br>'}
...
this.displayedResultBoxNum=stopIndex;$(html).appendTo(this.$resultBoxes).css({display:'block',opacity:0}).animate({opacity:1},{duration:600});},createResultBoxHtml:function(item){var s=this;var html='';if(item.view_type==1){html+='<div class="bus-search-result__box">';html+='<div class="bus-search-result__box--title">';html+='<div class="label--plum">直达巴士</div>';html+='<h2 class="bus-search-result-box__title text-em">从 '+item.start_place_chs+'</h2>';}else{html+='<div class="bus-search-result__box bus-search-result__box--gray">';html+='<div class="bus-search-result__box--title">';html+='<div class="label--black">非直达巴士</div>';html+='<h2 class="bus-search-result-box__title text-em">从 '+item.place_name_chs+'</h2>';}
html+='</div>'
html+='<div class="bus-search-result__box--data">';html+='<div class="bus-search-result__data--place">';html+='<h3 class="place--h">出发地点</h3>';if(item.view_type==1){html+='<p class="place--b">'+item.start_place_text_chs+'</p>';}else{html+='<p class="place--b">'+item.place_name_chs+'</p>';}
html+='</div>'
if(item.view_type==1){html+='<div class="bus-search-result__data--fare">';html+='<h3 class="place--h">车费</h3>';html+='<p class="place--b">'+s.TextReplace(item.fare_chs)+'</p>';html+='</div>';}
html+='<div class="bus-search-result__data--timetable">';html+='<table class="table--timetable">';html+='<tr>';html+='<th class="ch01" colspan="2">出发</th>';html+='<th class="ch02">抵达</th>';html+='</tr>';html+='<tr>';html+='<td class="cl01">'+item.departure_time.replace(/:00$/,''),+'</td>';html+='<td class="cl_arw">→</td>';html+='<td class="cl02">'+item.arrival_time.replace(/:00$/,'')+'</td>';html+='</tr>';html+='<tr>';if(this.condValue.dir==1){if(item.view_type==1){html+='<td>'+item.start_place_chs+'</td>';html+='<td class="cl_arw">→</td>';html+='<td>御殿场Premium Outlets&reg;</td>';}else{html+='<td>'+item.place_name_chs+'</td>';html+='<td class="cl_arw">→</td>';html+='<td>'+item.terminal_name_chs+'</td>';}}else{if(item.view_type==1){html+='<td>Gotemba Premium Outlets</td>';html+='<td class="cl_arw">→</td>';html+='<td>'+item.start_place_chs+'</td>';}else{html+='<td>'+item.place_name_chs+'</td>';html+='<td class="cl_arw">→</td>';html+='<td>'+item.terminal_name_chs+'</td>';}}
html+='</tr>';html+='</table>';html+='</div>';html+='<div class="bus-search-result__data--btn">';if(item.view_type==1){html+='<a href="../detail'+item.direct_bus_id+'.html" class="bus-search-btn">详细内容</a>';}else{html+='<a href="'+item.url_chs+'" target="_blank" class="bus-search-btn">查看运行巴士网站<img src="/_assets_rev03/images/common/previous/icon/icon_external_wh.png" alt="在新选项卡或窗口中打开"><img src="/_assets_rev03/images/common/previous/icon/icon_external.png" alt="在新选项卡或窗口中打开"></a>';}
