...
(function($){var BusSearch=function(){this.init();}
...
this.condition.show(this.condText);$.ajax({url:'/api/v1/gotemba/access/bus/search',type:'GET',dataType:'json',data:{'departure_date':this.condValue.date,'bus-search-time-begin':this.condValue.time_begin,'bus-search-time-end':this.condValue.time_end,'direction':this.condValue.dir,'search_area':this.condValue.place,'include_timeline':this.condValue.other}}).done(function(data){s.searchDone(data);}).fail(function(data){s.searchError(data);})},inpudValidate:function(){if(!this.$inputDate.val()){alert('請選擇出發日期');return false;}
var bt=this.$inputTimeBegin.val();var et=this.$inputTimeEnd.val();if(bt&&et){if(parseInt(bt.replace(':',''))>=parseInt(et.replace(':',''))){alert('請重新選擇出發時間');return false;}}
if(!this.$inputPlace.filter(':checked')[0]){alert('請至少選擇一個上車地點');return false;}
...
Condition.prototype={init:function($app){var s=this;this.$app=$app;this.$condition=this.$app.find('.js-bus-search-condition');},show:function(condText){var html='';html+='出發日期 : '+(condText.date||'Unspecified')+'<br>';if(!condText.dir){html+='希望乘車時間:Unspecified<br>'}else{if(!condText.time_begin&&!condText.time_end){html+='希望乘車時間 : '+condText.dir+' Unspecified<br>';}else{html+='希望乘車時間 : '+condText.dir+' '+(condText.time_begin||'')+' ~ '+(condText.time_end||'')+'<br>';}}
if(!condText.place[0]){html+='抵達時間 : Unspecified<br>';}else{html+='抵達時間 : '+condText.place.join('、')+'<br>'}
...
var html='';console.log(this.resultData);for(var i=startIndex;i<stopIndex;i++){html+=this.createResultBoxHtml(this.resultData.datalist[i]);}
this.displayedResultBoxNum=stopIndex;$(html).appendTo(this.$resultBoxes).css({display:'block',opacity:0}).animate({opacity:1},{duration:600});},createResultBoxHtml:function(item){var s=this;var html='';if(item.view_type==1){html+='<div class="bus-search-result__box">';html+='<div class="bus-search-result__box--title">';html+='<div class="label--plum">直達巴士</div>';html+='<h2 class="bus-search-result-box__title text-em">從 '+item.start_place_cht+'</h2>';}else{html+='<div class="bus-search-result__box bus-search-result__box--gray">';html+='<div class="bus-search-result__box--title">';html+='<div class="label--black">非直達巴士</div>';html+='<h2 class="bus-search-result-box__title text-em">從 '+item.place_name_cht+'</h2>';}
html+='</div>'
html+='<div class="bus-search-result__box--data">';html+='<div class="bus-search-result__data--place">';html+='<h3 class="place--h">出發地點</h3>';if(item.view_type==1){html+='<p class="place--b">'+item.start_place_text_cht+'</p>';}else{html+='<p class="place--b">'+item.place_name_cht+'</p>';}
html+='</div>'
if(item.view_type==1){html+='<div class="bus-search-result__data--fare">';html+='<h3 class="place--h">車費</h3>';html+='<p class="place--b">'+s.TextReplace(item.fare_cht)+'</p>';html+='</div>';}
html+='<div class="bus-search-result__data--timetable">';html+='<table class="table--timetable">';html+='<tr>';html+='<th class="ch01" colspan="2">出發</th>';html+='<th class="ch02">抵達</th>';html+='</tr>';html+='<tr>';html+='<td class="cl01">'+item.departure_time.replace(/:00$/,''),+'</td>';html+='<td class="cl_arw">→</td>';html+='<td class="cl02">'+item.arrival_time.replace(/:00$/,'')+'</td>';html+='</tr>';html+='<tr>';if(this.condValue.dir==1){if(item.view_type==1){html+='<td>'+item.start_place_cht+'</td>';html+='<td class="cl_arw">→</td>';html+='<td>御殿場Premium Outlets&reg;</td>';}else{html+='<td>'+item.place_name_cht+'</td>';html+='<td class="cl_arw">→</td>';html+='<td>'+item.terminal_name_cht+'</td>';}}else{if(item.view_type==1){html+='<td>Gotemba Premium Outlets</td>';html+='<td class="cl_arw">→</td>';html+='<td>'+item.start_place_cht+'</td>';}else{html+='<td>'+item.place_name_cht+'</td>';html+='<td class="cl_arw">→</td>';html+='<td>'+item.terminal_name_cht+'</td>';}}
html+='</tr>';html+='</table>';html+='</div>';html+='<div class="bus-search-result__data--btn">';if(item.view_type==1){html+='<a href="../detail'+item.direct_bus_id+'.html" class="bus-search-btn">詳細內容</a>';}else{html+='<a href="'+item.url_cht+'" target="_blank" class="bus-search-btn">查閱運行巴士網站<img src="/_assets_rev03/images/common/previous/icon/icon_external_wh.png" alt="在新分頁或視窗中開啟"><img src="/_assets_rev03/images/common/previous/icon/icon_external.png" alt="在新分頁或視窗中開啟"></a>';}
